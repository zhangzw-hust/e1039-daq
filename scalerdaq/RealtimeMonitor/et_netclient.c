...
int main(int argc, char **argv)
{
...
#endif
  int		 writeIt=1, printIt=1, control[]= {1,0,0,0};
  et_event       *evs[1000];
  ...
  */
   ...
   * and specify the port that the server is on.
    ...
    et_open_config_sethost(openconfig, "10.10.10.150");
    printf("debug2.3\n");
    //    et_open_config_setserverport(openconfig, 11222);
    et_open_config_setserverport(openconfig, 45400);
    printf("debug2.4\n");
  */
  ...
  et_station_config_setcue(sconfig, 1000);
  ...
  printf("Begin grabbing events...\n");
  /* 
     Notes by Kaz:  This is where the datastream readout begins.  
  */
  FILE *filehandle;
  filehandle = fopen("netclient.out","a");
  if (1) {
  ...
  for (i=0; i < 20000; i++) {
	...
	//        printf("%d ",*pInt);
	for(k=0;k<2000;k++){//output the first 20 array elements...an event, obviously, is longer than 20 words, but this is just a check.
	  //	  printf("%d %d %d \n",j,k,pInt[k]);
	  fprintf(filehandle, "%d %d %d %lu \n",totalcounter,j,k,pInt[k]);
	}
