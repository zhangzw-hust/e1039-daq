...
#    lines = ['<span color="red" size="%d"><b>Problem with EPICS on Target Computer</b></span>' % HEADER_TEXT_SIZE]
    ...
    lines.append( "Check the <a href='http://e906-gat3.fnal.gov:8081/SeaQuestDAQStatus/'>DAQ Status page</a> to see if the Target EPICS problem still exists." )
...
class StatusMonitorPrompt( Prompt ):
  ...
  def GetStandardLines( self ):
    ...
    lines.append( "Check the <a href='http://e906-gat3.fnal.gov:8081/SeaQuestDAQStatus/'>DAQ Status page</a> to see if the page is really updating." )
...
while True:
  ...
  print "oldestDir '{0}'".format(oldestDir) 
  print "oldestDirFull '{0}'".format(oldestDirFull) 

  oldestFile = sorted( os.listdir(oldestDirFull) )[-1]
  oldestFileFull = os.path.join( oldestDirFull, oldestFile )
  print "oldestFile '{0}'".format(oldestFile)  
  print "oldestFileFull '{0}'".format(oldestFileFull)  
