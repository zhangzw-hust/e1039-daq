...
void vme_ts_tir_scale__init (rolp)
     rolParam rolp;
{
...
void usrtrig(unsigned long EVTYPE,unsigned long EVSOURCE)
{
    long EVENT_LENGTH;
  {   
unsigned long ii, jj, idx_max, tmp_fifocount;
...
| (( 0x01 ) << 8) | (0xff & *(rol->nevents));	}	((rol->dabufp))++;} ;   *rol->dabufp++ = 0x11111111;
...
{ 
  ...
  if(event_ty == 14 && BeamOn == 1) {
    if(nEvents < 30000 && spillFull==0) {
      ...
      if(0  != 0) {
        scalarInfo[nEvents][0] = s32p[1]->scaler[~( 20 )&(31) ];  
        scalarInfo[nEvents][1] = s32p[1]->scaler[~( 21 )&(31) ];  
        scalarInfo[nEvents][2] = s32p[1]->scaler[~( 22 )&(31) ];  
        scalarInfo[nEvents][3] = s32p[1]->scaler[~( 23 )&(31) ];  
        scalarInfo[nEvents][4] = s32p[1]->scaler[~( 24 )&(31) ];  
        scalarInfo[nEvents][5] = s32p[0]->scaler[~( 28 )&(31) ];  
        scalarInfo[nEvents][6] = s32p[0]->scaler[~( 29 )&(31) ];  
        scalarInfo[nEvents][7] = s32p[0]->scaler[~( 30 )&(31) ];  
        scalarInfo[nEvents][8] = s32p[0]->scaler[~( 31 )&(31) ];  
        scalarInfo[nEvents][9] = s32p[0]->scaler[~( 14 )&(31) ];  
      }
      ++nEvents;
    }else{
          if (spillFull==0){
              tsEnableInput(0xe00, 0);        
logMsg("Event number is %i, exceed 30000.Throttle the trigger....\n",nEvents);           }
    ...
    if(nFlushes < 9000 ) {
      *rol->dabufp++ = 0xe906f00f;
      if((nFlushes - 1)* 6  < nEvents) {
        idx_max = nFlushes* 6 ;
        if(idx_max > nEvents) idx_max = nEvents;
        *rol->dabufp++ = (idx_max - (nFlushes-1)* 6 )*2 + 1;
        for(ii = (nFlushes-1)* 6 ; ii < idx_max; ++ii) {
      ...
      if(0  != 0) {
        *rol->dabufp++ = 0xe906f014;
        if((nFlushes - 1)* 6  < nEvents) {
          idx_max = nFlushes* 6 ;
          if(idx_max > nEvents) idx_max = nEvents;
          *rol->dabufp++ = (idx_max - (nFlushes-1)* 6 )*11 + 1;
          for(ii = (nFlushes-1)* 6 ; ii < idx_max; ++ii) {
  ...
  if(event_ty == 11 || event_ty == 12) {
    ...
    *rol->dabufp++ = 1;
    for(ii = 0; ii < 32; ii++) {
      *rol->dabufp++ = s32p[1]->scaler[~( ii )&(31) ];
    }
    *rol->dabufp++ = 0xe906f003;
    *rol->dabufp++ = 2;
    for(ii = 0; ii < 32; ii++) {
      *rol->dabufp++ = s32p[2]->scaler[~( ii )&(31) ];
    }
    tsScalClear(0x7ffff);
    for(ii = 0; ii < 3; ++ii) {
      scale32Enable(ii, 0);
