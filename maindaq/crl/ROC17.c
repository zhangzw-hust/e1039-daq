...
int UP_Limit = 266;
int Low_Limit = 116;
int PROP_UP_Limit = 210; 
int PROP_Low_Limit = 0;  
...
unsigned long res;
  bigendian_out = 0;
  tirInit(0x0ed0 );
LimitReg = Low_Limit + (UP_Limit << 16) + (TimeWindowOn << 31) + (ARM_enable << 27); PROP_LimitReg = PROP_Low_Limit + (PROP_UP_Limit << 16) + (PROP_TimeWindowOn << 31) + 
(ARM_enable << 27);   CR_Init(TDCBoardID, 0x1000000, 7 );
    ...
    for(ii = 0; ii < 7 ; ++ii) {
      ...
      *rol->dabufp++ = TDCBoardID + (ii << 24) + TDCHardID[ii];   
      ...
      if(ii > 3) {  
        ...
        *rol->dabufp++ = PROP_LimitReg;  
        CR_WR_Reg(ii, 5, PROP_LimitReg);  
        CR_WR_Reg(ii, 3, PROP_MultiHitSetup);  
      } else {
        *rol->dabufp++ = csr;   
...
{ 
  BeamOn = 1;
  for(ii = 0; ii < 7 ; ++ii) CR_WR_Reg(ii, 7, 0);
  for(ii = 0; ii <= 3; ++ii) {
    CR_p[ii]->reg[1] = csr;
...
if(((tmpaddr1 & 4) >> 2) != ((tmpaddr2 & 4) >> 2)) *rol->dabufp++ = 0xe906e906;         retVal = sysVmeDmaSend(rol->dabufp, DMAaddr, maxWords << 2, 0);
    ...
    for(ii = 0; ii < 7 ; ++ii) CR_WR_Reg(ii, 7, event_no);
  } else if(event_ty == 12) {     
    for(ii = 0; ii < 4; ++ii)    CR_FastTrigDisable(ii, csr);
    for(ii = 4; ii < 7 ; ++ii) CR_FastTrigDisable(ii, PROP_CSR);
    if(BeamOn == 1) {
      for(ii = 0; ii < 7 ; ++ii) DP_Write(ii, 0xe9060001, 0x7ffe, 0x7ffe);
    }
    nFlushes = 0;
    BeamOn = 0;
    ...
    for(ii = 4; ii < 7 ; ++ii) CR_FastTrigEnable(ii, PROP_CSR);
    if(BeamOn == 0) {
      for(ii = 0; ii < 7 ; ++ii) DP_Write(ii, 0xe9060000, 0x7ffe, 0x7ffe);
    }
    BeamOn = 1;
