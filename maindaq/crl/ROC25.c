     ...
     rolParam rolp;
{
      ...
      case 0 :
	{
	  ...
	  strcat(name, ":pool");
	  rolp->pool  = partCreate(name, 20000  , 4000 ,1);
	  if (rolp->pool == 0) {
	    rolp->inited = -1;
	    break;
	  }
...
StartOfUEvent = (rol->dabufp); *(++(rol->dabufp)) = ((( 132 ) << 16) | ( 0x01 ) << 8) | (0xff & 0 
  ...
  *rol->dabufp++ =rol->pid;
  ...
  for (ii=0;ii<NV1495;ii++){ 
    if ( 0 !=0 && ii>=0 && ii<NV1495L0 ){
      V_DELAY[ii]=V_DELAY[ii]+0x1000;
    }
    if ( 0 !=0 && ii>=NV1495L0 && ii<(NV1495L0+NV1495L1) ){
      V_DELAY[ii]=V_DELAY[ii]+0x1000;
    }
    if ( 0 !=0 && ii==(NV1495L0+NV1495L1) ){
      V_DELAY[ii]=V_DELAY[ii]+0x1000;
    }
    if ( 0 !=0 && ii>=NV1495L0 && ii<(NV1495L0+NV1495L1) ){	
...
->part->free_cmd)) ( __user_event__ ->part->clientData); } } ;	} __user_event__ = (DANODE *) 0; }; ;     daLogMsg("INFO","Prestart  1");
  }   
...
void usrtrig(unsigned long EVTYPE,unsigned long EVSOURCE)
{
    long EVENT_LENGTH;
  {   
unsigned long ii, iii, event_no, data_addr, retVal, maxWords, nWords, remBytes, 
TmpV1495Count;  int TmpData;
 ...
 if (0 !=0 || 0 !=0 || 0 !=0 ){
  ...
  if( 0 !=0 ){
   for (ii=NV1495L0;ii<(NV1495L0+NV1495L1);ii++){ 	
     ...
     *rol->dabufp++ =   v1495CommonstopRead(ii, 0x2d, -5);  
  ...
  if( 0 !=0 ){
     ...
     *rol->dabufp++ =   v1495RevisionRead(ii);    
     TmpV1495Count=v1495TDCcount(ii); 
     *rol->dabufp++ = TmpV1495Count ;        
     *rol->dabufp++ =   v1495CommonstopRead(ii);  
     if (TmpV1495Count!=0xd1ad){      	
       for(iii=0;iii<TmpV1495Count;iii++){
 	 *rol->dabufp++ = v1495TDCReadout(ii,iii);
       }
