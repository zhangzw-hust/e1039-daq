...
int UP_Limit = 266;
int Low_Limit = 116;
int TDCHardID[6] = {111, 112, 113, 114, 115, 116};
...
unsigned long res;
  bigendian_out = 0;
  tirInit(0x0ed0 );
LimitReg = Low_Limit + (UP_Limit << 16) + (TimeWindowOn << 31) + (ARM_enable << 27);   CR_Init(TDCBoardID, 0x1000000, 5 );
...
if(((tmpaddr1 & 4) >> 2) != ((tmpaddr2 & 4) >> 2)) *rol->dabufp++ = 0xe906e906;         retVal = sysVmeDmaSend(rol->dabufp, DMAaddr, maxWords << 2, 0);
    ...
    for(ii = 0; ii < 5 ; ++ii) CR_FastTrigDisable(ii, csr);
    if(BeamOn == 1) {
