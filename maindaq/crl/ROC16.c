...
int UP_Limit = 478;
int Low_Limit = 416;
int TDCHardID[7] = {70, 74, 72, 126, 107, 114, 122};
...
unsigned long res;
  bigendian_out = 0;
  tirInit(0x0ed0 );
LimitReg = Low_Limit + (UP_Limit << 16) + (TimeWindowOn << 31) + (ARM_enable << 27);   CR_Init(TDCBoardID, 0x1000000, 7 );
...
if(((tmpaddr1 & 4) >> 2) != ((tmpaddr2 & 4) >> 2)) *rol->dabufp++ = 0xe906e906;         retVal = sysVmeDmaSend(rol->dabufp, DMAaddr, maxWords << 2, 0);
    ...
    for(ii = 0; ii < 7 ; ++ii) CR_FastTrigDisable(ii, csr);
    if(BeamOn == 1) {
    ...
    for(ii = 0; ii < 7 ; ++ii) CR_WR_Reg(ii, 7, event_no);
    for(ii = 0; ii < 7 ; ++ii) CR_FastTrigEnable(ii, csr);
    if(BeamOn == 0) {
