...
int UP_Limit = 478;
int Low_Limit = 416;
int TDCHardID[6] = {82, 83, 84, 85, 86, 87};
...
unsigned long res;
  bigendian_out = 0;
  tirInit(0x0ed0 );
LimitReg = Low_Limit + (UP_Limit << 16) + (TimeWindowOn << 31) + (ARM_enable << 27);   CR_Init(TDCBoardID, 0x1000000, 6 );
...
StartOfUEvent = (rol->dabufp); *(++(rol->dabufp)) = ((( 132 ) << 16) | ( 0x01 ) << 8) | (0xff & 0 
  ...
  *rol->dabufp++ = rol->pid;
  ...
  SL_Init(0x0f000000 );
  SL_DataInit();
  SL_DP_Write(0xe906000f, 0x7ffe, 0x7ffe);
 } 
...
{ 
  BeamOn = 1;
  for(ii = 0; ii < 6 ; ++ii) CR_WR_Reg(ii, 7, 0);
  SL_WR_Reg(7, 0);
  for(ii = 0; ii < 6 ; ++ii) {
    CR_p[ii]->reg[1] = csr;
  ...
  SL_DataInit();
  SL_Scalar_Switch(0);
  SL_DP_Write(0xe9068000 + 1542 /257*49, 0x7ffe, 0x7ffe);
 } 
    ...
    for(ii = 0; ii <= 6 ; ++ii) {
      ...
      if(ii < 6 ) {
        ...
        *rol->dabufp++ = TDCBoardID + (ii << 24);
        ...
        maxWords = 48;
        DMAaddr = 0x0f000000  + 0x800;
        *rol->dabufp++ = 0xe906f013;
      }
      if(0  == 1) {
        tmpaddr1 = rol->dabufp;
        tmpaddr2 = DMAaddr;
if(((tmpaddr1 & 4) >> 2) != ((tmpaddr2 & 4) >> 2)) *rol->dabufp++ = 0xe906e906;         retVal = sysVmeDmaSend(rol->dabufp, DMAaddr, maxWords << 2, 0);
    ...
    for(ii = 0; ii < 6 ; ++ii) CR_WR_Reg(ii, 7, event_no);
    SL_WR_Reg(7, event_no);
    SL_Scalar_Switch(0);
  } else if(event_ty == 12) {     
    for(ii = 0; ii < 6 ; ++ii) CR_FastTrigDisable(ii, csr);
    SL_Scalar_Switch(1);
    if(BeamOn == 1) {
      for(ii = 0; ii < 6 ; ++ii) DP_Write(ii, 0xe9060001, 0x7ffe, 0x7ffe);
      SL_DP_Write(0xe9060001, 0x7ffe, 0x7ffe);
    }
    nFlushes = 0;
    BeamOn = 0;
    ...
    for(ii = 0; ii < 6 ; ++ii) CR_WR_Reg(ii, 7, event_no);
    SL_WR_Reg(7, event_no);
    for(ii = 0; ii < 6 ; ++ii) CR_FastTrigEnable(ii, csr);
    SL_Scalar_Switch(0);
    if(BeamOn == 0) {
      for(ii = 0; ii < 6 ; ++ii) DP_Write(ii, 0xe9060000, 0x7ffe, 0x7ffe);
      SL_DP_Write(0xe9060000, 0x7ffe, 0x7ffe);
    }
    BeamOn = 1;
    ...
    for(ii = 0; ii <= 6 ; ++ii) {
      if(nFlushes < 9000 ) {
        if(ii == 6 ) {
          Cnt = SL_DP_Read(0);
          DMAaddr = 0x0f000000  + 0x20000;
          *rol->dabufp++ = 0xe906f01b;
          *rol->dabufp++ = (0x0f000000  & 0xffff0000) + Cnt;
        } else {
          Cnt = DP_Read(ii, 0);
...
if(((tmpaddr1 & 4) >> 2) != ((tmpaddr2 & 4) >> 2)) *rol->dabufp++ = 0xe906e906;           retVal = sysVmeDmaSend(rol->dabufp, DMAaddr, Cnt << 2, 0);
        ...
        if(ii < 6 ) {
          ...
          SL_DP_Write(0xe9060002, 0x7ffe, 0x7ffe);
        }
      } else {
        if(ii < 6 ) {
          ...
          *rol->dabufp++ = DP_Read(ii, 0x7ffa);
          ...
          SL_DP_Write(0xe9060003, 0x7ffe, 0x7ffe);
          *rol->dabufp++ = 0xe906f019;
          *rol->dabufp++ = 0x0f000000 ;
          *rol->dabufp++ = SL_DP_Read(0x7ffa);
          SL_DP_Write(0, 0x7ffa, 0x7ffa);
        }
      }
    }
