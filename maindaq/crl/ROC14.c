...
int UP_Limit = 216;
int Low_Limit = 116;
int TRIG_MultiHitSetup = 0;
int TRIG_TimeWindowOn  = 1;
int TRIG_UP_Limit      = 200 ;
int TRIG_Low_Limit     = 100;
int TDCHardID[5] = {93, 92, 95, 97, 98};
...
unsigned long res;
  bigendian_out = 0;
  tirInit(0x0ed0 );
LimitReg = Low_Limit + (UP_Limit << 16) + (TimeWindowOn << 31) + (ARM_enable << 27); TRIG_LimitReg = TRIG_Low_Limit + (TRIG_UP_Limit << 16) + (TRIG_TimeWindowOn << 31) + 
(ARM_enable << 27);   CR_Init(TDCBoardID, 0x1000000, 5 );
...
StartOfUEvent = (rol->dabufp); *(++(rol->dabufp)) = ((( 132 ) << 16) | ( 0x01 ) << 8) | (0xff & 0 
    ...
    *rol->dabufp++ = rol->pid;
    for(ii = 0; ii < 5 ; ++ii) {
      ...
      *rol->dabufp++ = csr;   
      if(ii == 4) {  
        *rol->dabufp++ = TRIG_MultiHitSetup;
        *rol->dabufp++ = TRIG_LimitReg;  
        CR_WR_Reg(ii, 5, TRIG_LimitReg);  
        CR_WR_Reg(ii, 3, TRIG_MultiHitSetup);  
      } else if((ii == 2) || (ii == 3)) {   
LimitReg = Low_Limit+2 + ((UP_Limit+2) << 16) + (TimeWindowOn << 31) + (ARM_enable << 27);         *rol->dabufp++ = MultiHitSetup;
        *rol->dabufp++ = LimitReg;
        CR_WR_Reg(ii, 5, LimitReg);
        CR_WR_Reg(ii, 3, MultiHitSetup);  
      } else {   
LimitReg = Low_Limit + (UP_Limit << 16) + (TimeWindowOn << 31) + (ARM_enable << 27);         *rol->dabufp++ = MultiHitSetup;
        *rol->dabufp++ = LimitReg;
        CR_WR_Reg(ii, 5, LimitReg);
        CR_WR_Reg(ii, 3, MultiHitSetup);  
      }
...
{ 
  BeamOn = 1;
  for(ii = 0; ii < 5 ; ++ii) CR_WR_Reg(ii, 7, 0);
  for(ii = 0; ii < 5 ; ++ii) {
    CR_p[ii]->reg[1] = csr;
...
if(((tmpaddr1 & 4) >> 2) != ((tmpaddr2 & 4) >> 2)) *rol->dabufp++ = 0xe906e906;         retVal = sysVmeDmaSend(rol->dabufp, DMAaddr, maxWords << 2, 0);
    ...
    for(ii = 0; ii < 5 ; ++ii) CR_WR_Reg(ii, 7, event_no);
  } else if(event_ty == 12) {     
logMsg("Received EOS event_ty = %d , BeamOn = %d event_no = %d \n",event_ty, BeamOn, event_no);     for(ii = 0; ii < 5 ; ++ii) CR_FastTrigDisable(ii, csr);
    if(BeamOn == 1) {
      for(ii = 0; ii < 5 ; ++ii) DP_Write(ii, 0xe9060001, 0x7ffe, 0x7ffe);
    }
    nFlushes = 0;
    BeamOn = 0;
    ...
    for(ii = 0; ii < 5 ; ++ii) CR_WR_Reg(ii, 7, event_no);
    for(ii = 0; ii < 5 ; ++ii) CR_FastTrigEnable(ii, csr);
    if(BeamOn == 0) {
      for(ii = 0; ii < 5 ; ++ii) DP_Write(ii, 0xe9060000, 0x7ffe, 0x7ffe);
    }
    BeamOn = 1;
